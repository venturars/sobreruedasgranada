...
import createLaPintaRentRequest from "../../logic/create-la-pinta-rent-request.ts";
import { SMTPServerError, UnexpectedError } from "../../tools/errors";
import type { AstroComponentFactory } from "astro/runtime/server/index.js";

type LaPintaPostHandle = (
  Astro: Readonly<
    AstroGlobal<
      Record<string, any>,
...
const laPintaPostHandle: LaPintaPostHandle = async (Astro) => {
  if (Astro.request.method === "POST") {
    try {
      ...
      const contractAgreement = Boolean(
        data.get("contractAgreement") as string
      );
...
export default laPintaPostHandle;
